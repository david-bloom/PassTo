...
Canonical display rules live in `docs/design_system/WALLET_PASS_DISPLAY_SPEC.md`. Apple Wallet layout is implemented in the PassKit payload generated by `api/sign-apple.js`; Google Wallet mirrors the same canonical content through `api/sign-google.js`.
...
| `normalized_status` | `licenses.normalized_status` | headerFields.status_badge | textModulesData.status |
| `status_intent` | `licenses.status_intent` | display treatment input | display treatment input |
| `wallet_pass_treatment` | `licenses.wallet_pass_treatment` | display treatment input | display treatment input |
| `issue_date` | `licenses.issue_date` | auxiliaryFields.issued | textModulesData.issued |
| `expiration_date` | `licenses.expiration_date` | auxiliaryFields.expires | textModulesData.valid_through |
| `status_checked_at` | `licenses.status_checked_at` | backFields.last_verified | textModulesData.last_verified |
| `verification_source_display` | derived from `licenses.lookup_source` | backFields.source | textModulesData.source |
| `credential_created` | `credentials.created_at` | fallback for auxiliaryFields.issued | fallback for textModulesData.issued |
